...
\end{theorem}
\begin{proof}	
Since $\mathrm{P}(\tau<\infty)=1, X_{\tau \wedge n} \rightarrow X_{\tau}$ w.p. 1 and $\left|X_{\tau}\right| \leq K<\infty$ and hence $\mathrm{E}\left[\left|X_{\tau}\right|\right]<\infty .$ Thus, $\mathrm{E}\left[\left|X_{\tau}-X_{\tau \wedge n}\right| \right]\leq 2 K \mathrm{P}(\tau>n) \rightarrow 0$.
\end{proof}
...
\begin{proposition}[Hilbert spaces $\mathscr{M}^2$ and $\mathscr{M}^2_c$]
Let $\mathcal{C}_0$ denote the collection of all the martingales $M$ defined on the filtered probability space $(\Omega,\mathcal{F},(\mathcal{F}_{t})_{t\ge0},\mathrm{P})$ with initial value $M_0=0$ a.s.. All the $L^2$-bounded martingales $M\in\mathcal{C}_0$ constitute a Hilbert space, which is denoted by $\mathscr{M}^2$, with the inner product defined as 
$$
 (M, N)_{\mathscr{M}^2}:=\left( M_{\infty}, N_{\infty}\right)_{L^{2}}=\mathrm{E}\left[M_{\infty} N_{\infty}\right].
$$
All the $L^2$-bounded continuous martingales $M\in\mathcal{C}_0$ constitute a Hilbert space $\mathscr{M}^2_c$, which is a closed subspace of $\mathscr{M}^2$.
\end{proposition}
...
\begin{enumerate}
	\item $Z_0=0$ a.s;
	\item The collection $\left\{Z_{T}\mid T \text { a finite-valued stopping time}\right\}$ is is uniformly integrable.
\end{enumerate}
...
\begin{proposition}
	If $M\in \mathscr{M}^2_c$, then its quadratic variation $\langle M\rangle$ exists and has finite variation. The almost sure limit of $\langle M\rangle_{t}$ as $t \rightarrow \infty$ exists and is denoted by
	\[
...
\end{proposition}
\begin{proposition}
	If $M,N\in\mathscr{M}^2_c$, then $\langle M, N\rangle$ exists and $MN-\langle M, N\rangle$ is a uniformly integrable martingale. Consequently, the almost sure limit of $\langle M, N\rangle_{t}$ as $t \rightarrow \infty$ exists and is denoted by
	\[
...
\begin{proposition}[Hilbert space $\mathscr{L}^2(M)$]
	Suppose $M\in \mathscr{M}^2_c$. Define
	\begin{align*}
...
\begin{proposition}
For every $M \in \mathscr{M}^2_c, \mathscr{E}$ is dense in $\mathscr{L}^{2}(M)$.
\end{proposition}

\begin{theorem}
Let $M \in \mathscr{M}^2_c .$ For every $\Phi \in \mathscr{E}$ of the form
\[
\Phi_{t}(\omega)=\sum_{i=0}^{p-1} \Phi_{(i)}(\omega) \mathbf{1}_{\left(t_{i}, t_{i+1}\right]}(t)
\]
the formula
...
\]
defines a process $\Phi \cdot M \in \mathscr{M}^2_c.$ The mapping
\begin{align*}
I_M:\mathscr{L}^{2}(M)&\longrightarrow\mathscr{M}^2_c\\
\Phi &\mapsto \Phi \cdot M
\end{align*}
is a linear isometry, which means
\[
...
\]
Furthermore, $\Phi \cdot M$ is the unique martingale in $\mathscr{M}^2_c$ that satisfies the property
\begin{align*}
\langle \Phi \cdot M, N\rangle&= \Phi \cdot\langle M, N\rangle, \quad \forall N \in\mathscr{M}^2_c,\\
\left\langle \int_{0}^{t}\Phi_sdM_s, N_t\right\rangle&= \int_{0}^{t}\Phi_sd\left\langle M_s, N_s\right\rangle, \quad \forall N \in\mathscr{M}^2_c,\;t\in[0,\infty).
\end{align*}
We call $H \cdot M$ the stochastic integral of $H$ with respect to $M$.
\end{theorem}

\begin{proposition}
	Assume that $M,N\in\mathscr{M}^2_c$, $\Phi\in\mathcal{L}^2(M)$, $\Psi\in\mathcal{L}^2(N)$. Then
	\[
	\left\langle\int_{0}^\cdot \Phi_{s} \mathrm{d} M_{s}, \int_{0}^\cdot \Psi_{s} \mathrm{d} N_{s}\right\rangle_{t}=\int_{0}^{t} \Phi_{s}\Psi_{s} \mathrm{d}\langle M, N\rangle_{s}.
	\]
\end{proposition}

\begin{proposition}
If $\tau$ is a stopping time, we have
\begin{align*}
