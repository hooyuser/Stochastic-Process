...
\begin{definition}[discrete-time Markov chain]
	A \emph{discrete-time Markov chain} on a countable state space $S$ is a sequence of random variables $X_1, X_2, X_3,\cdots$ with the Markov property, namely that $\forall n\ge1,\ \forall j,i_1,\cdots,i_n\in S$,	
	\[
	\mathrm{P}(X_{n+1}=j\mid X_{1}=i_{1},X_{2}=i_{2},\cdots,X_{n}=i_{n})=\mathrm{P}(X_{n+1}=j\mid X_{n}=i_{n}), 
...
\noindent A discrete-time Markov chain $(X_n)_{n\ge1}$ is \emph{time-homogeneous} if
\[
\mathrm{P}(X_{n+2}=j\mid X_{n+1}=i)=\mathrm{P}(X_{n+1}=j\mid X_{n}=i)
\]
for all $n\ge1$ and all $i,j\in S$. We only focus on a discrete-time time-homogeneous Markov chain on a countable space if nothing is specified. 

\noindent One can consider the \emph{transition matrix}	$P=(p_{ij})_{i,j\in S}$, where
\[
...
Let $(X_n)_{n\ge1}$ be a discrete-time Markov chain on a countable state space $S$. The \emph{Chapman–Kolmogorov equation} states that
\[
...
\]
\end{proposition}
Of course $P^{(1)}=P$. Thus by iteration we show $P^{(n)}$ coincides with $P^n$.
...
\]
for all $s,t\ge0$ and all $i,j\in S$. We only focus on a continuous-time time-homogeneous Markov chain on a countable space if nothing is specified.
